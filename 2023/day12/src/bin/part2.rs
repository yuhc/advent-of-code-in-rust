...
fn solution(row: Vec<char>, cond: Vec<usize>) -> u64 {
    let m = cond.len();
    let n = row.len();
...
fn main() {
    ...
    for line in reader.lines() {
        ...
        let mut row: Vec<char> = row_str.chars().collect();
        row.push('?');
        let len = row.len() * 5 - 1;
        let mut row_dup = row.into_iter().cycle().take(len).collect::<Vec<_>>();

        let cond: Vec<usize> = cond_str.split(',').map(|s| s.parse::<usize>().unwrap()).collect();
        let len = cond.len() * 5;
        let cond_dup = cond.into_iter().cycle().take(len).collect::<Vec<_>>();

        let ans = solution(row_dup, cond_dup);
        println!("Result: {}", ans);
        tot += ans;
    }
