...
fn bfs(bricks: &[Brick]) {
    ...
    for i in 0..n {
        ...
        let mut k = 0;
        queue.push(i);
        let mut is_falling: Vec<bool> = vec![false; n];
        is_falling[i] = true;
        ...
        while k < queue.len() {
            ...
            // Remove this brick from the bricks that are supported by it.
            for supported in &bricks[b].supporting {
                if !is_falling[*supported]
                    && bricks[*supported]
                        .supported_by
                        .iter()
                        .all(|s| is_falling[*s])
                {
                    is_falling[*supported] = true;
...
fn main() {
    ...
    // Update bricks' supporting dependency.
