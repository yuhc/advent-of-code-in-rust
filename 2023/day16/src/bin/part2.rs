...
fn solution(graph: &Vec<Vec<char>>, start_x: usize, start_y: usize, start_dir: Direction) -> u32 {
    let n = graph.len();
    let m = graph[0].len();
    ...
    visited[start_x][start_y][start_dir as usize] = true;
    let mut i = 0;
...
fn main() {
    ...
    // From top and bottom.
    for j in 0..m {
        let tot = solution(&graph, 0, j, Direction::South);
        max_tot = std::cmp::max(max_tot, tot);
        let tot = solution(&graph, n - 1, j, Direction::North);
        max_tot = std::cmp::max(max_tot, tot);
    }
    // From left and right.
    for i in 0..n {
        let tot = solution(&graph, i, 0, Direction::East);
        max_tot = std::cmp::max(max_tot, tot);
        let tot = solution(&graph, i, m - 1, Direction::West);
        max_tot = std::cmp::max(max_tot, tot);
    }
    println!("Max: {}", max_tot);
}
